...
file_clear_property() {
	...
	if sed -i -E "s/${fcp_property}=(\[.+\] )?/${fcp_property}=/" "$fcp_filepath" >>"$logfile"; then
...
file_set_property() {
	...
	if sed -i -E "s@$fsp_property=.*@$fsp_property=$fsp_value_escaped@" "$fsp_filepath"; then
...
file_prepend_value_to_property() {
	...
	if sed -i -E "s/$favtp_property=/&$favtp_value/" "$favtp_filepath"; then
		echo " [INFO] File property was set successfully." >>"$logfile"
	else
		error_add "sed.setprop"
		echo " [ERR!] File property set failed." >>"$logfile"
...
file_remove_xml_key_value() {
	...
	if sed -i -E "/<$frxkv_key>/s/,? *$frxkv_value//g" "$frxkv_filepath"; then
...
file_add_xml_key_value() {
	...
	if sed -i -E "/<$faxkv_key>/s/<$faxkv_key>(.*)<\/$faxkv_key>/<$faxkv_key>\1,$faxkv_value<\/$faxkv_key>/" "$faxkv_filepath"; then
